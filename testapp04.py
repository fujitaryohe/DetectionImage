...
def main():
    ...
    # タイトルの表示
    st.title("日本三大ラーメン鑑定士アプリ")
    st.write("アプリの概要")
    st.write("このアプリは日本三大ラーメン(喜多方　札幌　博多)を判別するアプリです。")
    ...
    if uploaded_file is not None:
        ...
        prediction = teachable_machine_classification(image, 'ra-men_model.h5')        
        ...
        if classNo == 0:
            st.subheader(f"これは{pred0}％の確率で「喜多方ラーメン」です！")
        elif classNo == 1:
            st.subheader(f"これは{pred1}％の確率で「札幌ラーメン」です！")
        else:
            st.subheader(f"これは{pred2}％の確率で「博多ラーメン」です！")
...
# 学科：高度情報工学科
# 学年：４年
# コメント：teachable machineでいろんなことが出来るのがとても楽しかった。
#=============================================================================#
